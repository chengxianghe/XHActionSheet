...
- (IBAction)onClick:(id)sender {
    ...
    XHActionSheet *action = [XHActionSheet showInView:self.view title:@"退出登录" cancelTitle:@"取消" otherTitles:@[@"test",@"确定",@"无标题",@"test3",@"test4",@"test5"] custom:^(XHActionSheet *actionSheet, UILabel *titleLabel) {
    ...
    [action changeActionSheetTitle:@"点击test无反应"];
}

- (void)nextAction {
    __weak typeof(self)weakSelf = self;
    [XHActionSheet showInView:self.view title:@"退出登录后不会删除任何历史数据, 下次登录依然可以使用本账号, 请牢记你的账号密码~" cancelTitle:@"取消" otherTitles:@[@"退出登录",@"确认退出",@"test2",@"test3",@"test4",@"push到下个界面"] custom:^(XHActionSheet *actionSheet, UILabel *titleLabel) {
...
// 空的
- (void)nextAction2 {
    [XHActionSheet showInView:nil title:nil cancelTitle:@"取消" otherTitles:@[@"test"] custom:nil clickIndex:nil cancel:nil];
}
